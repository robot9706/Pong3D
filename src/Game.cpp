...
"   DiffuseValue = min(1, max(0, dot(normalize(LD), vNorm)) + 0.65);"
"}";
...
Game::Game(Pong3D* p)
{
    ...
    LoadTexture(_menuBG, "Data/MenuBG.png");

    _menu = new Texture2D();
    LoadTexture(_menu, "Data/Menu.png");

    _pongLogo = new Texture2D();
    LoadTexture(_pongLogo, "Data/PongLogo.png");

    _startBTN = new Texture2D();
    LoadTexture(_startBTN, "Data/StartBTN.png");

    _okBTN = new Texture2D();
    LoadTexture(_okBTN, "Data/OkBTN.png");

    string pKeys[] = {"P1Keys.png","P2Keys.png","P3Keys.png", "P4Keys.png"};
    _pButtons = new Texture2D[4];
    for(int x = 0;x<4;x++){
        LoadTexture(&_pButtons[x], "Data/" + pKeys[x]);
    }

    string pHelp[] = {"P1Movement.png","P2Movement.png","P3Movement.png", "P4Movement.png"};
    _pButtonHelp = new Texture2D[4];
    for(int x = 0;x<4;x++){
        LoadTexture(&_pButtonHelp[x], "Data/" + pHelp[x]);
    }

     string pNames[] = {"P1.png","P2.png","P3.png", "P4.png"};
    _pNames = new Texture2D[4];
    for(int x = 0;x<4;x++){
        LoadTexture(&_pNames[x], "Data/" + pNames[x]);
    }

    _botBTN = new Texture2D();
    LoadTexture(_botBTN, "Data/BotBTN.png");

    _winz = new Texture2D();
    LoadTexture(_winz, "Data/Winz.png");

    _xBTN = new Texture2D();
    LoadTexture(_xBTN, "Data/XButton.png");

    string numz[] = {"Num0.png", "Num1.png", "Num2.png", "Num3.png", "Num4.png", "Num5.png", "Num6.png", "Num7.png", "Num8.png", "Num9.png"};
    _numbers = new Texture2D[10];
    for(int x = 0;x<10;x++){
        LoadTexture(&_numbers[x], "Data/" + numz[x]);
    }

    string pws[] = {"BiggerPad.png","SmallerPad.png","FasterBall.png","SlowerBall.png","DoubleBall.png"};
    _powerupImages = new Texture2D[5];
    for(int x = 0;x<5;x++){
        LoadTexture(&_powerupImages[x], "Data/" + pws[x]);
    }
...
void Game::LoadTexture(Texture2D* tex, string file)
{
    if(!tex->Load(file))
    {
        cout<<"Hiba \"" << file << "\" betöltése közben!" << endl;
        Pong3D::GlobalError = true;
    }
}
...
{
    return ((_p1Ready || _p1Bot) && (_p2Ready || _p2Bot) && (_p2Ready || _p2Bot) && (_p2Ready || _p2Bot));
}
...
void Game::DrawGame()
{
    ...
    //Nézet beállítások
    glm::mat4 p = glm::perspective(45.0f, ((float)_pong->GetGFX()->GetContextWidth() / (float)_pong->GetGFX()->GetContextHeight()), 1.0f, 100.0f);
    glm::mat4 v = glm::lookAt(glm::vec3(camRenderX * 1.5f + camLookX, 12, camRenderZ * 1.5f), glm::vec3(camLookX,-1,0), glm::vec3(0,1,0));
